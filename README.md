...
greenness moderates ground heat.
...
pipeline does not beat ERA5-only, the contribution is *forecastability* — running
...
inference/                  the models and the pipeline  <- the product
  anga_pro.py                 the four chained models
  ...
  smoke_test.py               verifies a deployed run against reference output
scripts/
  evaluate_baselines.py       the number that could disprove us
  build_counties_geojson.py   county boundaries from geoBoundaries
config/
  regions.yaml               region definitions: counties, climate zone, geometry file
