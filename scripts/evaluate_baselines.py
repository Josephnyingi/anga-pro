"""
Score the bridge model pipeline against the baselines it has to beat.
...
make it meaningful:
  ...
  ERA5-only        the same classifier on atmospheric features alone, no bridges.
                   This is the one that tests the pipeline's actual thesis: if the
...
def evaluate(region: str, label_override: str = None) -> dict:
    ...
    bridge = fit_and_score(train, test, feature_columns(df, label, False), label)
    era5_only = fit_and_score(train, test, feature_columns(df, label, True), label)

    lift = round(bridge["f1"] - era5_only["f1"], 3)
    logger.info(f"{region}: bridge F1 {bridge['f1']} vs ERA5-only {era5_only['f1']} "
                f"(lift {lift:+})")
    ...
    return {
        ...
        "confidence_interval": {
            ...
            **wald_interval(bridge["accuracy"], len(test)),
        },
        "baselines": {
        ...
        "bridge_lift_f1_over_era5_only": lift,
    }


def main():
    logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")
    parser = argparse.ArgumentParser(description="Bridge model vs baselines")
    ...
    for r in results:
        print(f"{r['region']:<16}{r['base_rate']:>7.3f}"
              ...
              f"{r['metrics']['f1']:>8.3f}{r['bridge_lift_f1_over_era5_only']:>+8.3f}")
    print(f"\nwrote {out}")
